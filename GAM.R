...
gam_model2 <- gam(log(Elytra.length) ~ Sex * Anthro_numeric1 + Habitat.type +
...
gam_model3 <- gam(log(Pronotum.length) ~ 
                   Sex * Anthro_numeric +
                   log(Elytra.length) +
                   s(Region, bs = "re"), weights = Predicted.sex, family=gaussian(link="identity"),
                 data = df, method = "REML")
...
gam_model4 <- gam(log(Head.length) ~ 
                   Sex * Anthro_numeric1 +
                  log(Elytra.length)*Sex +
                   s(Region, bs = "re"), weights = Predicted.sex, family=gaussian(link="identity"),
                 data = df, method = "REML")
summary(gam_model4)
...
gam_model5 <- gam(log(Elytra.width) ~ 
                   Sex * Anthro_numeric1 +
                   log(Elytra.length) * Anthro_numeric1 +
